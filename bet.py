...
# === ЗАПУСК ===
# === АВТОМАТИЧЕСКАЯ РАССЫЛКА В 18:00 ===
def send_daily_picks():
    ...
    if not picks:
        ...
        return
    response = f"📊 ЕЖЕДНЕВНЫЙ ПРОГНОЗ на {datetime.now().strftime('%d.%m.%Y')} (18:00):\n\n"
    for i, pick in enumerate(picks, 1):
        response += f"{i}. {pick['match']}\n   🎯 {pick['bet']} (коэф. {pick['odds']})\n   💰 {pick['stake']:,.0f} ₸\n\n"
...
if __name__ == "__main__":
    ...
    print("⏰ Ежедневная рассылка настроена на 18:00")

    # Планируем ежедневную рассылку в 18:00
    schedule.every().day.at("18:00").do(send_daily_picks)
